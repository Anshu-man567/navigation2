...
//  RCLCPP_INFO(node_->get_logger(), "the parameters are - cost %f | %f ", planner_->costmap_tolerance_, planner_->euc_tolerance_);
//  node_->get_parameter(name_ + ".bias", planner_->bias_);
}
...
void LazyThetaStarP_Planner::getPlan(nav_msgs::msg::Path & global_path) {
  ...
  /// NOTE : a few lines of code has to be removed in the final version
  ...
  if (!(planner_->isSafe(planner_->src.x,
	...
	RCLCPP_INFO(node_->get_logger(), "COULD NOT GENERATE PLAN");
	global_path.poses.clear();
  }
  ...
  RCLCPP_INFO(node_->get_logger(), "the cost at src : %i",
			  planner_->costmap_->getCost(planner_->src.x+1, planner_->src.y+1));
  RCLCPP_INFO(node_->get_logger(), "the cost at dst : %i",
			  planner_->costmap_->getCost(planner_->dst.x+1, planner_->dst.y+1));
}
